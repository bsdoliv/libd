...
#define D_ENSURE_PARSED(a) \
    do { \
        if (!(a)->d->isParsed) \
            (a)->d->parse(); \
    } while (0)

#define d_sockaddr_in6 sockaddr_in6
#define d_s6_addr s6_addr
...
public:
    ...
    void setAddress(const D_IPV6ADDR &a_);
    ...
    D_IPV6ADDR a6; // IPv6 address
    AbstractSocket::NetworkLayerProtocol protocol;

    friend class HostAddress;
};

HostAddressPrivate::HostAddressPrivate()
    : a(0), isParsed(true), protocol(AbstractSocket::UnknownNetworkLayerProtocol)
{
    memset(&a6, 0, sizeof(a6));
}

void HostAddressPrivate::setAddress(quint32 a_)
{
    a = a_;
    ...
    a6[11] = 0xFF;
    a6[10] = 0xFF;
    protocol = AbstractSocket::IPv4Protocol;
    isParsed = true;
}

static bool parseMappedAddress(quint32& a, const D_IPV6ADDR &a6)
{
    int i;
    for (i=0;i<10;i++)
...
void HostAddressPrivate::setAddress(const quint8 *a_)
{
    for (int i = 0; i < 16; i++)
        a6[i] = a_[i];
    a = 0;
    if (parseMappedAddress(a, a6))
        ...
        protocol = AbstractSocket::IPv6Protocol;
    isParsed = true;
}

void HostAddressPrivate::setAddress(const D_IPV6ADDR &a_)
{
    a6 = a_;
    a = 0;
    if (parseMappedAddress(a, a6))
...
bool HostAddressPrivate::parse()
{
    isParsed = true;
    protocol = AbstractSocket::UnknownNetworkLayerProtocol;
...
bool NetmaskAddress::setAddress(const QString &address)
{
    length = -1;
    HostAddress other;
...
bool NetmaskAddress::setAddress(const HostAddress &address)
{
    ...
    HostAddress::operator=(address);

    if (d->protocol == AbstractSocket::IPv4Protocol) {
        ip.v4 = qToBigEndian(d->a);
        end = ptr + 4;
    } else if (d->protocol == AbstractSocket::IPv6Protocol) {
...
int NetmaskAddress::prefixLength() const
{
    return length;
}

void NetmaskAddress::setPrefixLength(AbstractSocket::NetworkLayerProtocol proto, int newLength)
{
    length = newLength;
    if (length < 0 || length > (proto == AbstractSocket::IPv4Protocol ? 32 :
                                proto == AbstractSocket::IPv6Protocol ? 128 : -1)) {
        // invalid information, reject
        d->protocol = AbstractSocket::UnknownNetworkLayerProtocol;
        length = -1;
        return;
    }
...
/*!
    Constructs a host address object with the IPv6 address \a ip6Addr.
*/
HostAddress::HostAddress(const D_IPV6ADDR &ip6Addr)
    : d(new HostAddressPrivate)
{
    setAddress(ip6Addr);
}
...
/*!
    ...
    \sa setAddress()
*/
HostAddress::HostAddress(const struct sockaddr *sockaddr)
    : d(new HostAddressPrivate)
{
    if (sockaddr->sa_family == AF_INET)
        setAddress(htonl(((sockaddr_in *)sockaddr)->sin_addr.s_addr));
    else if (sockaddr->sa_family == AF_INET6)
        setAddress(((d_sockaddr_in6 *)sockaddr)->sin6_addr.d_s6_addr);
}
...
HostAddress::HostAddress(const HostAddress &address)
    : d(new HostAddressPrivate(*address.d.data()))
{
}
    ...
    : d(new HostAddressPrivate)
{
    ...
    case Any:
        d->clear();
        d->protocol = AbstractSocket::AnyIPProtocol;
        break;
    }
}
...
/*!
    ...
    \sa setAddress()
*/
HostAddress &HostAddress::operator=(const QString &address)
{
    setAddress(address);
    return *this;
}
...
*/
void HostAddress::setAddress(quint32 ip4Addr)
{
...
*/
void HostAddress::setAddress(quint8 *ip6Addr)
{
...
/*!
    ...
    Set the IPv6 address specified by \a ip6Addr.
*/
void HostAddress::setAddress(const D_IPV6ADDR &ip6Addr)
{
    d->setAddress(ip6Addr);
}
...
*/
bool HostAddress::setAddress(const QString &address)
{
    d->ipString = address;
    return d->parse();
}
...
*/
void HostAddress::setAddress(const struct sockaddr *sockaddr)
{
    clear();
    if (sockaddr->sa_family == AF_INET)
        setAddress(htonl(((sockaddr_in *)sockaddr)->sin_addr.s_addr));
...
/*!
    ...
    \sa toString()
*/
quint32 HostAddress::toIPv4Address() const
{
    D_ENSURE_PARSED(this);
    return d->a;
}
...
/*!
    ...
    \sa toString()
*/
D_IPV6ADDR HostAddress::toIPv6Address() const
{
    D_ENSURE_PARSED(this);
    return d->a6;
}
...
QString HostAddress::toString() const
{
    ...
    if (d->protocol == AbstractSocket::IPv6Protocol) {
        QString s;
...
{
    D_ENSURE_PARSED(this);
    return (d->protocol == AbstractSocket::IPv6Protocol) ? d->scopeId : QString();
}
...
{
    D_ENSURE_PARSED(this);
    D_ENSURE_PARSED(&other);

    if (d->protocol == AbstractSocket::IPv4Protocol)
        return other.d->protocol == AbstractSocket::IPv4Protocol && d->a == other.d->a;
    if (d->protocol == AbstractSocket::IPv6Protocol) {
        return other.d->protocol == AbstractSocket::IPv6Protocol
               && memcmp(&d->a6, &other.d->a6, sizeof(D_IPV6ADDR)) == 0;
    }
...
{
    D_ENSURE_PARSED(this);
    HostAddress otherAddress(other);
    D_ENSURE_PARSED(&otherAddress);

    if (d->protocol == AbstractSocket::IPv4Protocol)
        return otherAddress.d->protocol == AbstractSocket::IPv4Protocol && d->a == otherAddress.d->a;
    if (d->protocol == AbstractSocket::IPv6Protocol) {
        return otherAddress.d->protocol == AbstractSocket::IPv6Protocol
               && memcmp(&d->a6, &otherAddress.d->a6, sizeof(D_IPV6ADDR)) == 0;
    }
    if (d->protocol == AbstractSocket::AnyIPProtocol)
        return other == HostAddress::Any;
...
{
    D_ENSURE_PARSED(this);
    return d->protocol == AbstractSocket::UnknownNetworkLayerProtocol;
}
...
/*!
    ...
    subnet. The HostAddress member of \a subnet contains the network
...
QDebug operator<<(QDebug d, const HostAddress &address)
{
    d.maybeSpace() << "HostAddress(" << address.toString() << ')';
    return d.space();
}
#endif

uint qHash(const HostAddress &key, uint seed)
{
...
*/
QDataStream &operator<<(QDataStream &out, const HostAddress &address)
{
    qint8 prot;
    prot = qint8(address.protocol());
    out << prot;
    switch (address.protocol()) {
    case AbstractSocket::UnknownNetworkLayerProtocol:
    case AbstractSocket::AnyIPProtocol:
        break;
    case AbstractSocket::IPv4Protocol:
        out << address.toIPv4Address();
        break;
    case AbstractSocket::IPv6Protocol:
    {
        D_IPV6ADDR ipv6 = address.toIPv6Address();
        for (int i = 0; i < 16; ++i)
            out << ipv6[i];
        out << address.scopeId();
    }
...
/*! \relates HostAddress
    ...
    \sa {Serializing Qt Data Types}
*/
QDataStream &operator>>(QDataStream &in, HostAddress &address)
{
    qint8 prot;
    in >> prot;
    switch (AbstractSocket::NetworkLayerProtocol(prot)) {
    case AbstractSocket::UnknownNetworkLayerProtocol:
        address.clear();
        break;
    case AbstractSocket::IPv4Protocol:
